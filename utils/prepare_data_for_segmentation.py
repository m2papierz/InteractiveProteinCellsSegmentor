...
def merge_channels(data_dir: str, n_channels: int) -> np.ndarray:
    ...
    :param data_dir: utils directory
...
def preprocess_mask(mask_path: str) -> tf.Tensor:
    ...
    Note:
        Originally mask tensor has many unique values. Value equal to 29 represents cell instances and that is
        why conversion of mask tensor values into 0 and 1 is needed concerning image segmentation.
    ...
    mask = tf.where(mask == 29, np.dtype('uint8').type(1), np.dtype('uint8').type(0))
    return mask
...
def create_dataset_csv(data_path: str, image_path: str, mask_path: str, csv_save_path: str) -> None:
    """
    Create *.txt file for easy accessing segmentation utils. Data is organised
    in three columns: image_id, image_path, mask_path.

    :param data_path: utils directory
