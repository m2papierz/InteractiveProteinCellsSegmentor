...
class FCN:
    def __init__(
        ...
        conv5 = ReLU()(conv5)
        output_ = (Conv2D(
            filters=4096, kernel_size=(7, 7), kernel_initializer='he_normal', padding='same'))(conv5)

        conv7 = BatchNormalization()(output_)
        conv7 = ReLU()(conv7)
        conv7 = (Conv2D(
            filters=4096, kernel_size=(1, 1), kernel_initializer='he_normal', padding='same'))(conv7)
        conv7_up = Conv2DTranspose(
            filters=2, kernel_size=(4, 4), strides=(4, 4), use_bias=False)(conv7)

        conv4 = BatchNormalization()(conv4)
        conv4 = ReLU()(conv4)
        conv4_1x1 = (Conv2D(
            filters=2, kernel_size=(1, 1), kernel_initializer='he_normal', padding='same'))(conv4)
        pool4_up = (Conv2DTranspose(
            filters=2, kernel_size=(2, 2), strides=(2, 2), use_bias=False))(conv4_1x1)

        conv3 = BatchNormalization()(conv3)
        conv3 = ReLU()(conv3)
        conv3_1x1 = (Conv2D(
            filters=2, kernel_size=(1, 1), kernel_initializer='he_normal', padding='same'))(conv3)

        output_ = Add()([pool4_up, conv3_1x1, conv7_up])
        output_ = Conv2DTranspose(
            filters=2, kernel_size=(8, 8), strides=(8, 8), use_bias=False)(output_)
        output_ = Conv2D(
            1, kernel_size=(1, 1), activation="sigmoid")(output_)
